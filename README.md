...
!python plot_metrics.py \
  --run-dir /kaggle/working/runs/resnet18_tinyimagenet
```

图形输出目录：

```text
/kaggle/working/runs/resnet18_tinyimagenet/figures
```

生成的图：

- `accuracy_curves.png`：训练集和验证集 top-1/top-5 曲线。
- `loss_curves.png`：训练集和验证集 loss 曲线。
- `learning_rate.png`：学习率变化曲线。
- `epoch_time.png`：每个 epoch 的耗时。
- `attack_accuracy.png`：clean、FGSM、PGD 的 top-1/top-5 对比。
...
runs/resnet18_tinyimagenet/figures/
```
